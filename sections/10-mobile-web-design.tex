...
React with TypeScript and Vite is used for the current web prototype. The screen
catalogue below describes the target interface; implemented interactions and
missing integrations are identified in the handover chapter.
...
The Photographer Application is a Flutter source prototype targeting mobile
platforms; no Android or iOS device build is claimed in this snapshot.
The target product allows photographers to find a Film Lab, create processing orders, track
...
Four target frontend sequence diagrams are documented in \path{docs/MobileWeb.md}.
The following summaries describe the proposed shared-API interactions:
...
\subsection{Prototype Screens and Presentation Plan}

The implemented prototype screens provide concrete interface examples: Photographer
Home, Lab details and booking preview, Orders, Archive, Marketplace, the Film Lab
dashboard, processing board, scan review, and administration queues. Source paths
and acceptance steps are listed in \path{docs/AcceptanceTraceability.md}.
These interactive prototypes are distinct from a separate low-fidelity wireframe
package; no additional wireframe files are claimed.
